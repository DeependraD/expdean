...
- Currently only 3 models are tidied (in join step) due to column name mismatch in `model_summary_kabletype.R`'s `p_annotator()` function. Use metaprogramming to solve it.

# Building my first package

- When hosting a package remotely as a source repo we can confirm that the name we choose for the package is infact available. Ensure this by:
